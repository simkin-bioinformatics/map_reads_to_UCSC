...
hub_name=snakemake.params.bam_hub_name
...
def make_header(hub_name, email, genome):
	output_file.write(f'hub {hub_name} bams\n')
	output_file.write(f'shortLabel {hub_name} bams\n')
	output_file.write(f'longLabel {hub_name} bams\n')
...
for sample in sample_list:
	name='.'.join(sample.split('/')[-1].split('.')[:-1]).strip('_sorted') #should work as long as sample follows snakemake format
	output_file.write(f'track {name}_bam\n')
	output_file.write(f'shortLabel {name}_bam\n')
	output_file.write(f'longLabel {hub_name}_{name}_bam\n')
	output_file.write(f'visibility hide\n')
	output_file.write(f'type bam\n')
...
print(f'bam hub complete. If your track is on a public browser, navigate to the'
...
print('Alternatively, if your track is on a private browser, you will need to'
	f' paste a URL formatted like this: {private_prefix}/bam_hub.txt - filling'
